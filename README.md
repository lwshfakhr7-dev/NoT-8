...
//
// FINAL SIGNAL:
// Two or more engines in the same direction = one confirmed signal.
//
// IMPORTANT:
// - No gold confirmation circles.
// - No pink confirmation circles.
// - No repeated same-direction signals.
// - Signals are created only after candle confirmation.
// - Reversal/Special context is allowed to contain opposite-colored
//   candles. The context is determined by the overall price direction.
// - 1m / 5m / 15m / 30m / 1H use minimum 4-candle context.
// - 4H uses minimum 3-candle context.
// - Engulfing patterns require minimum 3-candle context.
// - S/R is calculated independently on the current chart timeframe.
//=====================================================================
...
movingColor = emaBull ? color.yellow : color.rgb(255, 105, 105)
...
isHourTimeframe = timeframe.isminutes and timeframe.multiplier == 60
isThirtyMinute = timeframe.isminutes and timeframe.multiplier == 30
...
isFourHour = timeframe.isminutes and timeframe.multiplier == 240
...
reversalSpecialAllowed = supportedTimeframe
...
//=====================================================================
// 6. DIRECTIONAL CONTEXT
//
// The old logic required every candle to have the same color.
// That rule is intentionally removed.
//
// A trend context is valid when:
// - the minimum number of previous candles exists;
// - price has moved directionally over the context;
// - the directional candle count supports the context.
//
// Opposite-colored candles are allowed.
//=====================================================================
f_downContext(int barsCount) =>
    float netMove = close[1] - close[barsCount]
    int downCount = 0

    for i = 1 to barsCount
        downCount += close[i] < close[i + 1] ? 1 : 0

    bool valid = not na(close[barsCount + 1]) and netMove < 0 and downCount >= int(math.ceil(barsCount * 0.50))
    valid


f_upContext(int barsCount) =>
    float netMove = close[1] - close[barsCount]
    int upCount = 0

    for i = 1 to barsCount
        upCount += close[i] > close[i + 1] ? 1 : 0

    bool valid = not na(close[barsCount + 1]) and netMove > 0 and upCount >= int(math.ceil(barsCount * 0.50))
    valid


priorDownTrend =
     reversalSpecialAllowed and
     f_downContext(contextBars)

priorUpTrend =
     reversalSpecialAllowed and
     f_upContext(contextBars)


//=====================================================================
// 7. THREE-CANDLE CONTEXT FOR ENGULFING
//=====================================================================
f_downContextThree() =>
    float netMove = close[1] - close[3]
    int downCount = 0
...
hammer =
     priorDownTrend and
     lowerWick >= math.max(body * 2.0, syminfo.mintick * 2.0) and
     upperWick <= math.max(body * 0.50, syminfo.mintick)

invertedHammer =
     priorDownTrend and
     upperWick >= math.max(body * 2.0, syminfo.mintick * 2.0) and
     lowerWick <= math.max(body * 0.50, syminfo.mintick)
...
longLowerShadow =
     priorDownTrend and
     lowerWick >= math.max(body * 3.0, syminfo.mintick * 3.0)

longUpperShadow =
     priorUpTrend and
     upperWick >= math.max(body * 3.0, syminfo.mintick * 3.0)


//=====================================================================
// 9. ENGINE 1 FINAL DATABASE
//=====================================================================
bool reversalBuy = false
bool reversalSell = false
string reversalPattern = ""

if reversalSpecialAllowed
    if bullDojiReversal
        reversalBuy := true
        reversalPattern := "نموذج الدوجي"
    else if bearDojiReversal
        reversalSell := true
        reversalPattern := "نموذج الدوجي"
    else if hammer
        reversalBuy := true
        ...
        reversalBuy := true
        reversalPattern := "الاختراق الصاعد"
...
//=====================================================================
// 12. ENGINE 3 - SUPPORT & RESISTANCE
//
// The S/R database belongs only to the current chart timeframe.
// No higher-timeframe S/R levels are imported.
//=====================================================================
...
//---------------------------------------------------------------------
// Current active support/resistance
//---------------------------------------------------------------------
...
//=====================================================================
// 13. FINAL THREE-ENGINE CONFIRMATION
//
// Two engines are required in the same direction.
//=====================================================================
buyEngineCount =
     (reversalBuy ? 1 : 0) +
     (specialBuy ? 1 : 0) +
...
//=====================================================================
// 14. NO REPEATED SAME-DIRECTION SIGNAL
//=====================================================================
...
// 16. SIGNAL POSITION
//
// The signal is intentionally positioned relative to the candle BODY,
// not at the end of the wick.
//
// A moderate ATR distance is used so the signal remains visible without
// becoming part of the candle.
//=====================================================================
signalBodyDistance = atr * 0.22
signalBoxHeight = atr * 0.30

buyBodyLow = math.min(open, close)
sellBodyHigh = math.max(open, close)

buySignalBottom = buyBodyLow - signalBodyDistance - signalBoxHeight
buySignalTop = buyBodyLow - signalBodyDistance

sellSignalBottom = sellBodyHigh + signalBodyDistance
sellSignalTop = sellBodyHigh + signalBodyDistance + signalBoxHeight


//=====================================================================
// 17. FINAL SIGNAL BOXES
//
// The triangle is integrated into the label itself:
// BUY  = upward label shape.
// SELL = downward label shape.
//
// No separate triangle is created.
// No gold circle is created.
//=====================================================================
buySignalColor = color.rgb(0, 200, 45)
sellSignalColor = color.rgb(220, 25, 25)
...
// 18. DAILY STATISTICS
//
// Statistics reset at the beginning of each new trading day.
// A completed trade is successful after 120 symbol points in its
// direction. A move of the same distance against the entry is loss.
//=====================================================================
...
//=====================================================================
// 25. PIVOT VISUAL MARKERS
//
// These are only structural markers for confirmed pivots.
// They are not BUY/SELL signals.
//=====================================================================
plotshape(
     not na(pivotHigh),
     title="قمة مؤكدة",
